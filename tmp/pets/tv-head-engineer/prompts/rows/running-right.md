Create one horizontal animation strip for Codex pet `tv-head-engineer`, state `running-right`.
...
- This is not normal running. Show the mascot being lifted and dragged to the right by the mouse as a "picked up" pose, with the body slightly dangling rather than walking.
