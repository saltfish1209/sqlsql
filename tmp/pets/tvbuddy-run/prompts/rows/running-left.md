Create one horizontal animation strip for Codex pet `tvbuddy`, state `running-left`.
...
State action: Mouse-dragged-left loop: the pet is being lifted and dragged to the left by an unseen mouse, with a shocked emoji on the TV screen and a dangling picked-up body pose.
