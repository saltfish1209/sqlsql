...
if not os.path.isfile(INPUT_SCHEMA_FILE):
    _alt = os.path.join(DATA_DIR, "m_schema.txt")
    if os.path.isfile(_alt):
        INPUT_SCHEMA_FILE = _alt
    else:
        raise FileNotFoundError(
            f"Schema 文件未找到: 尝试了 {INPUT_SCHEMA_FILE} 和 {_alt}"
        )

TRAIN_OUTPUT_JSONL = os.path.join(DATA_DIR, "nl2sql_train_for_lora.jsonl")
VAL_OUTPUT_JSONL = os.path.join(DATA_DIR, "nl2sql_val_for_lora.jsonl")
TEST_OUTPUT_JSONL = os.path.join(DATA_DIR, "nl2sql_test_for_lora.jsonl")
...
# ---------------- Schema 加载 ----------------
def load_schema_meta(schema_path: str) -> list[dict]:
    """兼容 JSON 和 TXT 两种格式的 schema 加载。"""
    if schema_path.endswith(".json"):
        with open(schema_path, "r", encoding="utf-8") as f:
            return json.load(f)
    # TXT 格式：逐行解析 M-Schema
    import re as _re
    pattern = _re.compile(
