...
if [[ "$LLM_PROVIDER" == "ollama" ]]; then
  export OLLAMA_MODELS="/data2/yj/ollama/models"
  export OLLAMA_HOST="${OLLAMA_HOST:-${OLLAMA_HOST_BIND}:${OLLAMA_PORT}}"
  export OLLAMA_NUM_PARALLEL="${OLLAMA_NUM_PARALLEL:-1}"
  export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}"
  PORT="$OLLAMA_PORT"
  DEFAULT_BASE_URL="http://${OLLAMA_HOST_BIND}:${PORT}/v1"
  SERVED="${OLLAMA_MODEL_TAG:-${LLM_MODEL:-supergemma4-26b-uncensored-fast-v2-q4km}}"
